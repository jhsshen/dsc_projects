...
FROM Members
WHERE joindate = (
    SELECT MAX(joindate) AS latest_join_time
    FROM Members);
...
SELECT f.facid, f.name, DATE_FORMAT(b.starttime, '%Y-%m') AS month, SUM(b.slots) AS slots_booked_by_members_not_guests
FROM Facilities AS f
INNER JOIN Bookings AS b
ON f.facid = b.facid
WHERE b.memid <> 0
